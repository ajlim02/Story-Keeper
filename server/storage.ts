...
export interface IStorage {
  ...
  getRadarData(month: string, nation: Nation): Promise<RadarData | null>;
}
...
function calculateScore(momAbs: number, momPct: number, rankChange: number, allModels: Array<{ momAbs: number; momPct: number; rankChange: number }>): number {
  ...
  return 0.55 * z(momAbs, absArr) + 0.35 * z(Math.min(momPct, 500), pctArr) + 0.10 * z(rankChange, rankArr);
}
...
function processModelsWithScore(
  ...
  const processedModels = models.map((model) => {
    const score = risingModels.length > 0 
...
const dataStore: Record<string, Record<Nation, Omit<RadarModel, "score" | "danawaUrl">[]>> = {
  "2025-12": {
    domestic: domesticModels2025_12,
    export: exportModels2025_12,
  },
  "2025-11": {
    domestic: domesticModels2025_11,
    export: exportModels2025_11,
  },
};

export class MemStorage implements IStorage {
  async getAvailableMonths(): Promise<string[]> {
    return Object.keys(dataStore).sort().reverse();
  }

  async getRadarData(month: string, nation: Nation): Promise<RadarData | null> {
    ...
    const risingModels = models.filter((m) => m.momAbs > 0);
    const avgGrowth = risingModels.length > 0
